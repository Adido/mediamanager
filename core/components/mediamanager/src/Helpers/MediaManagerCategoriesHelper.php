...
class MediaManagerCategoriesHelper
{
    ...
    public function createCategory($name, $parent = 0, $sourceId = 0, $rank = 9999)
    {
        ...
        $category = $this->mediaManager->modx->getObject(MediamanagerCategories::class, [
            'name'             => $name,
            'parent_id'        => $parent,
        ...
        $category = $this->mediaManager->modx->newObject(MediamanagerCategories::class);
    ...
    {
        $category = $this->mediaManager->modx->getObject(MediamanagerCategories::class, (int) $id);
    ...
    public function deleteCategory($id, $newId)
    {
         ...
         */
        $q = $this->mediaManager->modx->newQuery(MediamanagerFilesCategories::class);
        $q->where([
            'mediamanager_categories_id:IN' => $categoryIds
        ]);

        $filesCategories = $this->mediaManager->modx->getCollection(MediamanagerFilesCategories::class, $q);
        $filesIds = [];
        ...
        foreach ($filesIds as $fileId) {
            $fileConnection = $this->mediaManager->modx->newObject(MediamanagerFilesCategories::class);
         ...
         */
        $this->mediaManager->modx->removeCollection(MediamanagerCategories::class, [
            'id:IN' => $categoryIds
        ]);
    ...
    public function sortCategories($items)
    {
        ...
        foreach ($items as $key => $value) {
            $category = $this->mediaManager->modx->getObject(MediamanagerCategories::class, $key);
    ...
    public function getCategories($source = null)
    {
        ...
        return $this->mediaManager->modx->getCollection(MediamanagerCategories::class, $q);
    }
    ...
    {
        $categories = $this->mediaManager->modx->getIterator(MediamanagerCategories::class, [
    ...
    {
        $query = $this->mediaManager->modx->newQuery(MediamanagerCategories::class);
        $query->where(['media_sources_id' => $this->mediaManager->sources->getCurrentSource()]);

        $query->sortby('parent_id', 'asc');

        $categories = $this->mediaManager->modx->getIterator(MediamanagerCategories::class, $query);
    ...
    private function getParents($categoryId, $list = [], $separator = ' > ') {
        if ($object = $this->mediaManager->modx->getObject(MediamanagerCategories::class, $categoryId)) {
