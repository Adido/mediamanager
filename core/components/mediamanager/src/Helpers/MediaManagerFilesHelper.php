...
namespace Sterc\MediaManager\Helpers;
...
use Sterc\MediaManager\Model\MediamanagerDownloads;
use Sterc\MediaManager\Model\MediamanagerFiles;
use Sterc\MediaManager\Model\MediamanagerFilesCategories;
...
use Sterc\MediaManager\Model\MediamanagerFilesMeta;
use Sterc\MediaManager\Model\MediamanagerFilesRelations;
use Sterc\MediaManager\Model\MediamanagerFilesTags;
use Sterc\MediaManager\Model\MediamanagerFilesVersions;
use Sterc\MediaManager\Model\MediamanagerTags;
use xPDO\xPDO;
...
class MediaManagerFilesHelper
{
    ...
    {
        // Get file
        $file = $this->mediaManager->modx->getObject(MediamanagerFiles::class, ['id' => $fileId]);

        // Get file categories
        $q = $this->mediaManager->modx->newQuery(MediamanagerCategories::class);
        $q->innerJoin(MediamanagerFilesCategories::class, 'Files');
        $q->where(['Files.mediamanager_files_id' => $fileId]);
        $q->groupby('Files.mediamanager_categories_id');

        $categories = $this->mediaManager->modx->getIterator(MediamanagerCategories::class, $q);

        // Get file tags
        $q = $this->mediaManager->modx->newQuery(MediamanagerTags::class);
        $q->innerJoin(MediamanagerFilesTags::class, 'Files');
        $q->where(['Files.mediamanager_files_id' => $fileId]);
        $q->groupby('Files.mediamanager_tags_id');

        $tags = $this->mediaManager->modx->getIterator(MediamanagerTags::class, $q);

        // Get file relations
        $q = $this->mediaManager->modx->newQuery(MediamanagerFiles::class);
        $q->innerJoin(MediamanagerFilesRelations::class, 'Relations');
        $q->where(['Relations.mediamanager_files_id_relation' => $fileId]);

        $relations = $this->mediaManager->modx->getIterator(MediamanagerFiles::class, $q);

        $q = $this->mediaManager->modx->newQuery(MediamanagerFiles::class);
        $q->innerJoin(MediamanagerFilesRelations::class, 'Relations2');
        $q->where(['Relations2.mediamanager_files_id' => $fileId]);

        $relations2 = $this->mediaManager->modx->getIterator(MediamanagerFiles::class, $q);

        // Get file content
        $q = $this->mediaManager->modx->newQuery(MediamanagerFilesContent::class);
        $q->where(['mediamanager_files_id' => $fileId]);

        $content = $this->mediaManager->modx->getIterator(MediamanagerFilesContent::class, $q);
    ...
    public function getFileHtml($fileId, $template)
    {
        ...
        // File history
        $v = $this->mediaManager->modx->newQuery(MediamanagerFilesVersions::class);
        $v->where([
            'mediamanager_files_id' => $fileId
        ]);
        $v->sortBy('version', 'desc');

        $versions = $this->mediaManager->modx->getIterator(MediamanagerFilesVersions::class, $v);
        ...
        foreach ($versions as $version) {
            ...
            if ($versionArr['action'] == 'replace') {
                if ($versionArr['replaced_file_id'] != 0) {
                    $oldFile = $this->mediaManager->modx->getObject(MediamanagerFiles::class, [
        ...
        $metaValues = $this->mediaManager->modx->getCollection(MediamanagerFilesMeta::class, ['mediamanager_files_id' => $fileId]);
        ...
        if ($licenseInfo = $data['file']->getLicense()) {
            ...
            foreach ($this->mediaManager->modx->newObject(MediamanagerFilesLicense::class)->toArray() as $key => $value) {
                $bodyData['licensing.' . $key] = $value;
            }
    ...
    public function getList($search = '', $filters = [], $sorting = [], $isArchive = 0, $limit = 50, $offset = 0)
    {
            ...
            foreach ($filters as $key => $value) {
                ...
                switch ($key) {
                    ...
                    case 'categories' :
                        ...
                        $q->innerJoin(MediamanagerFilesCategories::class, 'Categories');
                        $where[]['Categories.mediamanager_categories_id:IN'] = $value;
                        break;

                    case 'tags' :
                        $q->innerJoin(MediamanagerFilesTags::class, 'Tags');
        ...
        return $this->mediaManager->modx->getIterator(MediamanagerFiles::class, $q);
    }
    ...
    {
        $file = $this->mediaManager->modx->newObject(MediamanagerFiles::class);
        ...
        if ($file->save()) {
            ...
            if (isset($data['categories'])) {
                ...
                foreach ($data['categories'] as $category) {
                    if (is_numeric($category)) {
                        $object = $this->mediaManager->modx->newObject(MediamanagerFilesCategories::class);
            ...
            if (isset($data['tags'])) {
                ...
                foreach ($data['tags'] as $tag) {
                    if (is_numeric($tag)) {
                        $object = $this->mediaManager->modx->newObject(MediamanagerFilesTags::class);
            ...
            if (isset($data['meta']) && is_array($data['meta'])) {
                foreach ($data['meta'] as $key => $value) {
                    $object = $this->mediaManager->modx->newObject(MediamanagerFilesMeta::class);
            ...
            if (isset($data['license']) && is_array($data['license'])) {
                $object             = $this->mediaManager->modx->newObject(MediamanagerFilesLicense::class, $data['license']);
                ...
                if ($object->save()) {
                    $objectRelation = $this->mediaManager->modx->newObject(MediamanagerFilesLicenseFile::class, [
                        'mediamanager_files_id' => $file->get('id'),
    ...
    public function saveFile($fileId, $rawData)
    {
        ...
        $file = $this->mediaManager->modx->getObject(MediamanagerFiles::class, [
            'id' => $fileId
        ]);
        ...
        if (isset($mediaSource['licensing']) && $mediaSource['licensing'] === true) {
            ...
            if (!$license) {
                $license = $this->mediaManager->modx->newObject(MediamanagerFilesLicense::class);
            }
        ...
        // Save meta fields
        if (isset($data['meta']) && is_array($data['meta'])) {
            $this->mediaManager->modx->removeCollection(MediamanagerFilesMeta::class, [
                'mediamanager_files_id' => $file->get('id')
            ]);
        ...
        if (isset($mediaSource['licensing']) && $mediaSource['licensing'] === true) {
            ...
            if (isset($data['license']) && is_array($data['license'])) {
                ...
                if (!$this->mediaManager->modx->getObject(MediamanagerFilesLicenseFile::class, [
                    'mediamanager_files_id' => $file->get('id'),
                    'license_id'            => $license->get('id')
                ])) {
                    $licenseRelation = $this->mediaManager->modx->newObject(MediamanagerFilesLicenseFile::class, [
                        'mediamanager_files_id' => $file->get('id'),
                        'license_id'            => $license->get('id')
                    ]);
    ...
    public function saveFileVersion($fileId, $file, $action = '', $replacedId = 0) {
        ...
        $version = $this->mediaManager->modx->newObject(MediamanagerFilesVersions::class);
    ...
    {
        $version = $this->mediaManager->modx->getObject(MediamanagerFilesVersions::class, $versionId);
        if (!$version) {
            return [
                'status' => self::STATUS_ERROR,
                'message' => $this->alertMessageHtml(
        ...
        $file = $this->mediaManager->modx->getObject(MediamanagerFiles::class, $version->get('mediamanager_files_id'));
        if (!$file) {
            return [
                'status'  => self::STATUS_ERROR,
                'message' => $this->alertMessageHtml(
    ...
    public function deleteFiles($selectedFiles)
    {
        ...
        $files = $this->mediaManager->modx->getIterator(MediamanagerFiles::class, ['id:IN' => $fileIds]);
        if (!$files) {
            return false;
        }
        ...
        foreach ($files as $file) {
            ...
            $versions = $this->mediaManager->modx->getIterator(MediamanagerFilesVersions::class, ['mediamanager_files_id' => $file['id']]);
            foreach ($versions as $version) {
            ...
            $this->mediaManager->modx->removeCollection(MediamanagerFilesCategories::class, ['mediamanager_files_id' => $file['id']]);

            // Delete file tags
            $this->mediaManager->modx->removeCollection(MediamanagerFilesTags::class, ['mediamanager_files_id' => $file['id']]);

            // Delete file relations
            $this->mediaManager->modx->removeCollection(MediamanagerFilesRelations::class, ['mediamanager_files_id' => $file['id']]);
            $this->mediaManager->modx->removeCollection(MediamanagerFilesRelations::class, ['mediamanager_files_id_relation' => $file['id']]);
    ...
    {
        foreach ($selectedFiles as $file) {
            $fileCategory = $this->mediaManager->modx->getObject(MediamanagerFilesCategories::class, array(
                'mediamanager_files_id' => $file['id'],
                'mediamanager_categories_id' => $file['category']
            ));

            if (!$fileCategory) {
                $fileCategory = $this->mediaManager->modx->newObject(MediamanagerFilesCategories::class);
    ...
    public function archiveFiles($selectedFiles)
    {
        ...
        $q->select('
            ...
            Files.name AS name
        ');
        $q->innerJoin(MediamanagerFiles::class, 'Files');
        $q->where([
        ...
        foreach ($fileIds as $key => $id) {
            $file = $this->mediaManager->modx->getObject(MediamanagerFiles::class, $id);
    ...
    public function archiveReplaceFile($fileId, $newFileId)
    {
        ...
        // New file
        $newFile = $this->mediaManager->modx->getObject(MediamanagerFiles::class, array('id' => $newFileId));
        ...
        $linkedContent = $this->mediaManager->modx->getCollection(MediamanagerFilesContent::class, $q);
        ...
        foreach ($linkedContent as $fileContent) {
            if ($fileContent->get('is_tmplvar')) {
                ...
                if ($oldFile  = $this->mediaManager->modx->getObject(MediamanagerFiles::class, $fileId)) {
        ...
        $this->mediaManager->modx->updateCollection(MediamanagerFilesContent::class,
    ...
    public function shareFiles($selectedFiles, $isDownload = false)
    {
        ...
        // Check if files are linked to a resource
        $files = $this->mediaManager->modx->getIterator(MediamanagerFiles::class, array(
            'id:IN' => $fileIds
        ));
        ...
        $download = $this->mediaManager->modx->newObject(MediamanagerDownloads::class);
    ...
    public function cropFile($fileId, $cropData, $isNewImage = false)
    {
        ...
        $file = $this->mediaManager->modx->getObject(MediamanagerFiles::class, array('id' => $fileId));
    ...
    public function duplicateFile($file, $imageData)
    {
        ...
        // Get file categories
        $categories = $this->mediaManager->modx->getIterator(MediamanagerFilesCategories::class, array('mediamanager_files_id' => $file['id']));
        foreach ($categories as $category) {
            $data['categories'][] = $category->get('mediamanager_categories_id');
        }

        // Get file tags
        $tags = $this->mediaManager->modx->getIterator(MediamanagerFilesTags::class, array('mediamanager_files_id' => $file['id']));
        foreach ($tags as $tag) {
    ...
    {
        $file = $this->mediaManager->modx->newObject(MediamanagerFilesRelations::class);
    ...
    {
        $category = $this->mediaManager->modx->newObject(MediamanagerFilesCategories::class);
    ...
    {
        $this->mediaManager->modx->removeObject(MediamanagerFilesCategories::class, array(
            'mediamanager_files_id' => $fileId,
    ...
    public function addTag($fileId, $tagId, $name = false)
    {
        ...
        if ($tagId === 0 && $name !== false) {
            $tag = $this->mediaManager->modx->getObject(MediamanagerTags::class, [
                'name:='           => $name,
                'media_sources_id' => $this->mediaManager->sources->getCurrentSource()
            ]);
            if (!$tag) {
                $newTag = $this->mediaManager->modx->newObject(MediamanagerTags::class);
        ...
        // Add tag to file
        $tag = $this->mediaManager->modx->newObject(MediamanagerFilesTags::class);
    ...
    {
        $this->mediaManager->modx->removeCollection(MediamanagerFilesTags::class, array(
            'mediamanager_files_id' => $fileId,
    ...
    private function cleanupArchive()
    {
        ...
        // Remove files
        $this->mediaManager->modx->removeCollection(MediamanagerFiles::class, array(
            'is_archived'    => 1,
            'archive_date:<' => date('Y-m-d H:i:s', $time - ($cleanupMax * 86400))
        ));

        // Remove file categories
        $this->mediaManager->modx->removeCollection(MediamanagerFilesCategories::class, array(
        ...
        $this->mediaManager->modx->removeCollection(MediamanagerFilesRelations::class, array(
            'mediamanager_files_id:IN' => $fileIds,
